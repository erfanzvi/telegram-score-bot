...
def set_score(message):
    ...
    try:
        ...
        if user:
            telegram_id = user[0]
            cursor.execute(
                "UPDATE students SET score = ? WHERE student_id = ?", (score, student_id))
            conn.commit()
            # پیام خودکار به دانشجو
            bot.send_message(
                telegram_id, f"📢 نمره جدید ثبت شد!\n\n📊 نمره شما: {score}")
            bot.send_message(
                message.chat.id, "✅ نمره ثبت شد و برای دانشجو ارسال شد.")
        else:
            bot.send_message(
                message.chat.id, "❌ چنین شماره دانشجویی ثبت نشده.")
    except:
        bot.send_message(
